...
class Spl07_003:
    ...
    def __str_meas_cfg(self, bus, cfg):
    ...
    def __wait_for_sensor(self, bus, mask):
        ...
        # print(f'W: {self.__str_meas_cfg(bus, cfg)} (0x{mask:02x})')
        count = 0
        while count < 10:
            count += 1
            cfg = bus.read_byte(self.__address, _REG_MEAS_CFG)
            # print(f'W {count}: {self.__str_meas_cfg(bus, cfg)} (0x{mask:02x})')
            if cfg & mask == mask:
                break
    ...
    def write_cfg(self, bus):
        # bus.write_byte_data(self.__address, _REG_PRS_CFG, 0b00000001)
        bus.write_byte_data(self.__address, _REG_PRS_CFG, 0b00000100)
        bus.write_byte_data(self.__address, _REG_TMP_CFG, 0b00000000)
        # bus.write_byte_data(self.__address, _REG_CFG_REG, 0b00000000)
        bus.write_byte_data(self.__address, _REG_CFG_REG, 0b00000100)
    ...
    def measure_all(self, bus):
        ...
        while True:
            ...
            while count < 10000:
                ...
                cfg = bus.read_byte(self.__address, _REG_MEAS_CFG)
                print(f'P+T {count}: {self.__str_meas_cfg(bus, cfg)}')
    ...
    def __measure_pressure(self, bus):
        self.__wait_for_sensor(bus, 0b01000000)
        ...
        while count < 10:
            ...
            cfg = bus.read_byte(self.__address, _REG_MEAS_CFG)
            # print(f'P {count}: {self.__str_meas_cfg(bus, cfg)}')
            if cfg & 0x10:
                break
        else:
    ...
    def __measure_temperature(self, bus):
        self.__wait_for_sensor(bus, 0b01000000)
        ...
        while count < 10:
            ...
            cfg = bus.read_byte(self.__address, _REG_MEAS_CFG)
            # print(f'T {count}: {self.__str_meas_cfg(bus, cfg)}')
            if cfg & 0x20:
                break
        else:
    ...
    def measure(self, bus):
        return self.__calculate(self.__measure_pressure(bus), self.__measure_temperature(bus))
